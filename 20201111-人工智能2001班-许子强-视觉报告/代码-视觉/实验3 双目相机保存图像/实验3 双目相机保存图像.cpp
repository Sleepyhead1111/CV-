	...
	*/
	...
	while (true)
	{
		if (k4a_device_get_capture(device, &capture, K4A_WAIT_INFINITE) == K4A_RESULT_SUCCEEDED) {
			...
			int operate = cv::waitKey(1);
			printf("%d\n", operate);

			if (operate == 83 || operate == 115 || operate == 13)  // 按 S(ASCII==83)、s(115)、Enter(13) 保存图片
			{
				...
				imwrite("image\\rgb\\" + filename_rgb, cv_rgbImage_no_alpha);
				imwrite("image\\depth\\" + filename_d, cv_depth_8U);
				imwrite("image\\ir\\" + filename_ir, cv_irImage_8U);
			...
			uint16_t* depth_data = (uint16_t*)k4a_image_get_buffer(depthImage);
			...
			//以米（meters）为单位
			float max_distance = max_depth * 0.001f;
			std::cout << "最大深度（距离）为：" << max_distance << "米" << endl;
			...
			if (operate == 81 || operate == 113 || operate == 27)  // 按Q(ASCII==81)、q(113)、ESC(27)退出
				break;
